# export PYTHONPATH= $PYTHONPATH:/mnt/workspace/RetinexMamba/basicsr1/models/archs/vmamba_arch.py
...
from basicsr1.models.archs.SS2D_arch import SS2D
from basicsr1.models.archs.IFA_arch import IFA
import sys
...
class Illumination_Estimator(nn.Module):
    ...
    def __init__(self, n_fea_middle, n_fea_in=10, n_fea_out=3):
        """
...
class IGAB(nn.Module):
    ...
    def forward(self, x, illu_fea):
        ...
        for (trans, ss2d, ff) in self.blocks:
            ...
            # 应用SS2D模块并进行残差连接
            x = ss2d(y) + x.permute(0, 2, 3,
                                    1)  # 当我创建了一个类之后,如果继承nn并且自己定义了一个forward,那么nn会把hook挂到对象中,直接用对象(forward的参数)就能调用forward函数
...
class Denoiser(nn.Module):
    ...
    def __init__(self, in_dim=3, out_dim=3, dim=31, level=2, num_blocks=[2, 4, 4], d_state=16):
        ...
        for i in range(level):  # 每一级由于对应的特征维数不同，所以IGAB中的块数也不同，也要采用不同的头数
            self.encoder_layers.append(nn.ModuleList([
                ...
                # 由于特征维数加深，所以经过卷积要减少H和W的维数，相当于进行下采样,对应前馈中的FeaDownSample
                nn.Conv2d(dim_level, dim_level * 2, 4, 2, 1, bias=False)
                # 由于x的特征维数加深，导致分辨率下降，而x后面要进MSA，所以为了保持一直，fea也要进行对应的采样。对应前馈中的IlluFeaDownsample
            ]))
        ...
        self.bottleneck = IGAB(dim=dim_level, dim_head=dim, heads=dim_level // dim, num_blocks=num_blocks[-1],
                               d_state=d_state)  # 这一个IGAB对应4C特征维度的
        ...
        for i in range(level):
            self.decoder_layers.append(nn.ModuleList([
                ...
                # 反卷积，进行上采样，分辨率变大，特征维度变小，对应前馈中的FeaUpSample
                ...
                IGAB(dim=dim_level // 2, num_blocks=num_blocks[level - 1 - i], dim_head=dim,
                     heads=(dim_level // 2) // dim, d_state=d_state)
            ]))
            dim_level //= 2
            d_state //= 2
    ...
    def forward(self, x, illu_fea, base=None):
        """
        ...
        # 特征嵌入
        fea = self.embedding(x)  # 对应图中的F0特征向量x
        ...
        for i, (FeaUpSample, Fution, LeWinBlcok) in enumerate(self.decoder_layers):
            ...
            # print("")
            fea = Fution(
                torch.cat([fea, fea_encoder[self.level - 1 - i]], dim=1))  # 在这里直接将编码时对应的解码向量concatenate，然后送入卷积层
...
class ConvBlock(nn.Module):
    def __init__(self, in_channels=3, out_channels=3):
        ...
        self.main = nn.Sequential(
            ...
            # nn.BatchNorm2d(n_feature_2),
            nn.ReLU(True),
            nn.ConvTranspose2d(n_feature_2, out_channels, 5, 1, 2, bias=False),
        )
...
class GradDataFitting(nn.Module):
    ...
    def forward(self, Ol, On, g):
        """
        InPUTS:
               # g kernel N x C x h x w, tensor.float32
               # Ol Observed input N x C x H x W, tensor.float32
               # On is initialize of Ol/g, N x C x H x W, tensor.float32
        """
        res = g * On - Ol
        return res
...
class ADMMNet_Single_Stage(nn.Module):
    """
    单步 ADMM 展开：
    - 用网络直接求解 G 子问题
    - 用网络直接求解 On 子问题
    - 保留辅助变量与对偶变量更新
    """
    def __init__(self, in_channels=10, out_channels=3, n_feat=40, level=2, num_blocks=[1, 1, 1], d_state=16):
        super(ADMMNet_Single_Stage, self).__init__()

        # G 子问题求解器：直接输出 G^{k+1}
        self.g_solver = Illumination_Estimator(n_feat)

        # On 子问题求解器：直接输出 On^{k+1}
        self.on_solver = Denoiser(in_dim=in_channels, out_dim=out_channels, dim=n_feat, level=level, num_blocks=num_blocks, d_state=d_state)

        # Z_on, Z_g 对应隐式正则的prox/投影网络
        # 如果你想先简化，也可以先不用额外网络，直接闭式平均
        self.prox_on = ConvBlock()
        self.prox_g = ConvBlock()

        self.eps = 1e-6

        self.rho_on = nn.Parameter(torch.tensor(0.5))
        self.rho_g = nn.Parameter(torch.tensor(0.5))

    def positive(self, x):
        return F.softplus(x) + 1e-6

    def init_g_from_luminance(self, Ol):
        """
        用亮度图初始化 G
        Retinex 视角下，G 可看作 illumination / 亮度退化图的初值
        """
        if Ol.shape[1] == 3:
            # RGB -> luminance
            r = Ol[:, 0:1, :, :]
        ...
        # 扩展到 3 通道
        G0 = luminance.repeat(1, Ol.shape[1], 1, 1)
        G0 = torch.clamp(G0, 0.0, 1.0)
        return G0

    def init_state(self, Ol):
        """
        初始化状态：
        G0: 亮度图初值
        On0: 显式反演初值
        ...
        G0 = self.init_g_from_luminance(Ol).to(device=device, dtype=dtype)
        On0 = Ol / (G0 + self.eps)
        On0 = torch.clamp(On0, 0.0, 1.0)

        Z_on = On0.clone()
        Z_g = G0.clone()

        U_on = torch.zeros_like(Ol, device=device, dtype=dtype)
        U_g = torch.zeros_like(Ol, device=device, dtype=dtype)
    ...
    def forward(self, Ol, state=None, step_size=1.0):
        """
        单次依赖 step_size 的 ADMM 风格更新
        注意：
        1. 不做“一步更新后再插值”
        2. step_size 直接进入 G/On 子问题求解器
        3. step_size 直接进入 Z/U 更新
        4. 外部做 shortcut:
             state_big = model(Ol, state0, step_size=1.0)
             state_h1  = model(Ol, state0, step_size=0.5)
             state_h2  = model(Ol, state_h1, step_size=0.5)
           约束 state_big ≈ state_h2
        """
        if state is None:
            state = self.init_state(Ol)
        ...
        rho_on = self.positive(self.rho_on)
        rho_g = self.positive(self.rho_g)

        # 统一只用 step_size
        step_map = torch.full_like(Ol[:, :1, :, :], float(step_size))

        # ============================================================
        # 1) G 子问题: G^{k+1} = S_G(Ol, On^k, Z_g^k-U_g^k, step_size)
        # ============================================================
        G_input = torch.cat([Ol, On_k, Z_g_k - U_g_k, step_map], dim=1)
        illu_fea, G = self.g_solver(G_input)
        G = torch.clamp(G, 0.0, 1.0)

        # ============================================================
        # 2) On 子问题: On^{k+1} = S_On(Ol, G^{k+1}, Z_on^k-U_on^k, step_size)
        # ============================================================
        On_input = torch.cat([Ol, G, Z_on_k - U_on_k, step_map], dim=1)
        On = self.on_solver(On_input, illu_fea, base=On_k)
        On = torch.clamp(On, 0.0, 1.0)

        Z_on = (Ol * Z_g_k + rho_on * (On + U_on_k)) / (Z_g_k * Z_g_k + rho_on + self.eps)

        Z_on = torch.clamp(Z_on, 0.0, 1.0)

        Z_g = (Ol * Z_on + rho_g * (G + U_g_k)) / (Z_on * Z_on + rho_g + self.eps)
        Z_g = torch.clamp(Z_g, 0.0, 1.0)
...
class ADMMNet(nn.Module):
    def __init__(self, in_channels=10, out_channels=3, n_feat=31, stage=3, num_blocks=[1, 1, 1], d_state=16):
        super(ADMMNet, self).__init__()

        self.stages = nn.ModuleList([
            ADMMNet_Single_Stage(
    ...
    def forward(self, x, state=None, step_size=1.0):
        """
        x: 固定观测 Ol
        state: ADMM 状态；若为 None，则由第一个 stage 初始化
        step_size: 供 shortcut 使用
        """
        cur_state = state
        for i, stage in enumerate(self.stages):
            if i == 0:
                cur_state = stage(x, state=cur_state, step_size=step_size)
            else:
                cur_state = stage(x, state=cur_state, step_size=step_size)
        return cur_state
...
class ADMMNet_iter(nn.Module):
    def __init__(self, in_channels=10, out_channels=3, n_feat=31, stage=3, num_blocks=[1, 1, 1], d_state=16):
        super(ADMMNet_iter, self).__init__()

        self.iter = stage

        self.stage_block = ADMMNet_Single_Stage(
                in_channels=in_channels,
                out_channels=out_channels,
                n_feat=n_feat,
                level=2,
    ...
    def forward(self, x, state=None, step_size=1.0):
        ...
        for _ in range(self.iter):
            cur_state = self.stage_block(x, state=cur_state, step_size=step_size)
...
if __name__ == '__main__':
    ...
    model = ADMMNet(stage=1, n_feat=40, num_blocks=[1, 2, 2]).cuda()
    print(model)
    inputs = torch.randn((1, 3, 256, 256)).cuda()
    # k = torch.randn(1, 1, 45, 45).cuda()
    # kt = torch.randn(1, 1, 45, 45).cuda()
    model(inputs)
    ...
    # 常见的论文表示方式
    flops_g = flops.total() / 1e9  # GFLOPs
    params_m = n_param / 1e6  # MParams

    print(f'FLOPs: {flops_g:.2f} GFLOPs')
    print(f'Params: {params_m:.2f} MParams')