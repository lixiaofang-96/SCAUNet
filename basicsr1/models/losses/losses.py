...
from basicsr1.models.losses.loss_util import weighted_loss
import basicsr1.models.losses.pytorch_ssim as pytorch_ssim
...
class MyLoss(nn.Module):
    def __init__(self, lam_mse=60, lam_ssim=60, reduction='mean'):
    ...
    def forward(self, g, On, Ol, Ogt, k=None, weight=None):
        loss=0.0
        ssim_loss = pytorch_ssim.SSIM()
        loss_1 = l1_loss(On, Ogt, weight, reduction=self.reduction)
        loss_2 = 1 - ssim_loss(On, Ogt)
        recon = g * Ogt
        loss_3 = l1_loss(recon, Ol, weight, reduction=self.reduction)
        loss_4 = 1 - ssim_loss(recon, Ol)
        res = On - Ogt
        res = res - torch.min(res) / (torch.max(res) - torch.min(res))
        loss_5 = self.l_color(res)
        loss += self.lam_mse * loss_1 + self.lam_ssim * loss_2 + self.lam_mse * loss_3 + self.lam_ssim * loss_4
        return loss
