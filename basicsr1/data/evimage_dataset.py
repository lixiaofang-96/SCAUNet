...
class Dataset_EVImage(data.Dataset):
    """
    ...
    Training:
        - each __getitem__ first picks one scene
        - then randomly samples one image from that scene folder
        - GT is loaded from Label/<scene_name>.xxx
    ...
    def __init__(self, opt):
        ...
        # training virtual expansion
        self.virtual_repeat = opt.get('virtual_repeat', 4)
    ...
    def _is_image_file(self, fname):
        fname = fname.lower()
        return fname.endswith(('.png', '.jpg', '.jpeg', '.bmp', '.tif', '.tiff', '.JPG', '.PNG'))
    ...
    def _build_train_index(self):
        # one scene repeated virtually many times
        samples = []
        for _ in range(self.virtual_repeat):
