...
# 去掉当前模型中不存在的旧参数
drop_keys = ['beta_on', 'beta_g', 'eta_on', 'eta_g']
state_dict = {
    k: v for k, v in state_dict.items()
    if not any(k.endswith(name) for name in drop_keys)
}

# 不要补 module.，因为此时 model_restoration 还没有 DataParallel
msg = model_restoration.load_state_dict(state_dict, strict=False)
